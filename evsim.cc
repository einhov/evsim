...
int evsim(int argc, char **argv) {
	...
	glfwSetFramebufferSizeCallback(window, [](auto, int width, int height) {
		state.camera.aspect_ratio(width / static_cast<float>(height));
	...
	state.pause = false;
	state.draw = conf.get_boolean_default("draw", true);
	state.generation = 0;
	state.step = 0;
	state.tick = 0;
	state.draw_wall = 1;
